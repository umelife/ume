...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const { data: listing } = await supabase.from('listings').select('title, description, price, image_urls').eq('id', id).single()
  if (!listing) return { title: 'Listing | UME' }
  const price = (listing.price / 100).toFixed(2)
  const description = `Buy ${listing.title} for $${price} from a verified student. ${(listing.description || '').slice(0, 120)}`
  const image = listing.image_urls?.[0]
  return {
    title: `${listing.title} — UME Campus Marketplace`,
    description,
    openGraph: {
      title: `${listing.title} — UME Campus Marketplace`,
      description,
